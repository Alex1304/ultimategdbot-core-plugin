...
@CommandDescriptor(
		aliases = { "help", "manual" },
		shortDescription = "tr:strings.core/help_desc"
)
class HelpCommand {
	
	@CommandAction
	@CommandDoc("tr:strings.core/help_run")
	...
	private static Mono<Void> displayCommandList(Context ctx) {
		var sb = new StringBuilder(ctx.translate("strings.core", "command_list", ctx.prefixUsed()) + "\n\n");
	...
	private static Mono<Void> displayCommandDocumentation(Context ctx, String commandName, String subcommand) {
		...
		return Mono.justOrEmpty(ctx.bot().service(CommandService.class).getCommandByAlias(commandName))
				.switchIfEmpty(Mono.error(new CommandFailedException(ctx.translate("strings.core", "error_command_not_found", commandName))))
				...
				.flatMap(function((subcommands, cmd) -> {
					...
					if (!selectedSubcommand.isEmpty()) {
						return Mono.error(new CommandFailedException(
								ctx.translate("strings.core", "error_subcommand_not_found", selectedSubcommand, commandName) + '\n'
										+ "Available subcommands:\n" + formattedSubcommands));
					}
					return Mono.error(new CommandFailedException(
							ctx.translate("strings.core", "error_subcommand_required", commandName) + '\n'
									+ formattedSubcommands));
				}))
				.map(cmd -> formatDoc(
						ctx,
	...
	private static String formatDoc(Translator tr, Command cmd, String prefix, String flagPrefix, String selectedCommand, String selectedSubcommand) {
		...
		var sb = new StringBuilder(code(prefix + selectedCommand))
				...
				.append(selectedSubcommand.isEmpty() ? "" : "\n" + tr.translate("strings.core", "subcommand") + " " + code(selectedSubcommand))
				.append("\n\n")
				.append(bold(underline(tr.translate("strings.core", "syntax"))))
				.append("\n")
		...
		if (!entry.getFlagInfo().isEmpty()) {
			sb.append("\n").append(bold(underline(tr.translate("strings.core", "flags")))).append("\n");
		...
		if (doc.getEntries().size() > 1) {
			sb.append("\n").append(bold(underline(tr.translate("strings.core", "see_also")))).append("\n");
