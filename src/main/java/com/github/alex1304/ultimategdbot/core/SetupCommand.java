...
class SetupCommand {
	...
	private static Mono<Void> handleEditInteraction(Context ctx, List<GuildConfigurator<?>> configurators,
		...
		if (reset) {
			sb.append(ctx.translate("CoreStrings", "prompt_feature_reset"));
		} else {
			sb.append(ctx.translate("CoreStrings", "prompt_feature_setup"));
		}
		sb.append("**\n\n");
		var i = 1;
		for (var configurator : configurators) {
		...
		return ctx.bot().service(InteractiveMenuService.class)
				...
				.addMessageItem("", selectInteraction -> {
					...
					try {
						...
						return Mono.error(new UnexpectedReplyException(ctx.translate("CoreStrings", "error_invalid_input")));
					}
					if (selected < 1 || selected > configurators.size()) {
						return Mono.error(new UnexpectedReplyException(ctx.translate("CoreStrings", "error_invalid_input", selected)));
					}
